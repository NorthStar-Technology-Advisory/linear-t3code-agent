...
Each status requires a nonempty multiline-capable prompt and an output: `comment`, `specification`, `tickets`, `draft-pr`, `agent-managed` or `external-review`. `agent-managed` follows the status prompt for direct Linear publication and handoffs, without bridge artifact publication or a bridge-result block. `external-review` waits for the signed GitHub webhook without creating a T3Code thread or turn; it retains the implementation thread and PR association. `required-skills` defaults to an empty list and checks each named skill against the selected provider's workspace catalog. `new-thread` defaults to false and has no effect on `external-review`. Optional project-wide `instructions` supplement each status prompt. Provider/model/workspace preferences continue to inherit from T3Code; service credentials, concurrency and polling remain instance settings.
...
Output contracts preserve existing behavior: comment publishes the summary to the parent (or current issue without a parent); specification requires a revised parent description; tickets requires an approved child breakdown with acceptance criteria/dependencies; draft-pr requires passing reported validation and a verified open draft PR. The bridge continues to publish planning artifacts durably and protect human edits. A separately configured signed GitHub webhook may advance CodeRabbit reviews to implementation or UAT and mark issues Done after their matching PR merges; it never delegates children or merges PRs.
