...
export function deliveryResult(text: string, workflow: Stage["workflow"] = "implement") {
  const planning = workflow !== "implement";
  ...
  if (workflow === "to-spec" && !result.artifacts?.specification) return { complete: false, body: "Incomplete: a reviewable parent specification is required in the artifact output." };
  if (workflow === "to-tickets" && (!result.artifacts?.children || !result.artifacts.approved)) return { complete: false, body: "Incomplete: an approved breakdown of linked children with acceptance criteria and dependencies is required." };
  if (result.artifacts && ((workflow !== "to-spec" && result.artifacts.specification) || (workflow !== "to-tickets" && result.artifacts.children))) return { complete: false, body: "Incomplete: artifact output belongs to another workflow. Finish only the current human-selected stage." };
...
Never advance statuses, delegate children or approve your own stage output. Humans select the next stage. Honor reviews required by the installed skill. On re-entry revise existing artifacts by their retained identities, reconcile unstarted children, and flag impacts on active/completed children for human review without changing their commitments. Preserve existing repository files, branch and PR state across thread changes. A fresh thread does not authorize duplicate work or reopening a closed/merged PR.`;
  if (stage.workflow === "implement") return `${deliveryInstructions}\n${artifactInstructions}\nInvoke $implement (operator-installed skill at ${stage.skillPath}).`;
  return `${sharedInstructions}\n${artifactInstructions}
Invoke $${stage.workflow} (operator-installed skill at ${stage.skillPath}) for this stage only. Stage invocation: ${stage.id}.
${stage.workflow === "grill-me" ? "Ask repeated native user-input questions; wait without a deadline. Record agreed decisions and remaining uncertainties." : stage.workflow === "to-spec" ? "Synthesize a reviewable specification in the parent description, preserving the original problem." : "After skill-required human review, reconcile a breakdown as linked Linear children with acceptance criteria and blocking dependencies. Creating children must not start implementation."}
