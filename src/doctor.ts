...
export async function doctor(values: NodeJS.ProcessEnv, args: string[]): Promise<number> {
  ...
  try {
    ...
    report("FAIL", "Linear installation", reason === "scopes" ? "Required app scopes are missing. Reinstall the existing app with read, write, app:assignable and app:mentionable scopes." : reason === "authentication" ? "Authentication rejected. Reinstall through the existing /linear/install flow; preserve the database." : reason === "identity" ? "Identity is not the installed app. Complete actor=app OAuth using the existing application." : reason === "schema" ? "Identity API or scopes incompatible. Check Linear read scope and update the adapter." : reason === "connection" || error instanceof TypeError || error instanceof Error && ["TimeoutError", "AbortError"].includes(error.name) ? "Connection failed. Check access to api.linear.app, then rerun doctor." : "Missing, invalid or expired installation. Start the bridge and complete /linear/install; for expiry let the running bridge refresh, then rerun doctor. Doctor never refreshes or writes tokens.");
  }
